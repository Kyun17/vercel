...
import { fetchComments, createComments, deleteComments, updateComments } from '../../api/commentApi';
...
function PostComments({ postId }) {
    ...
    const checkEdit = (authorId) => {
        ...
        console.log("로그인유저ID:", me?.id, "댓글작성자ID:", authorId);
        return (
            !meIsLoading &&
            me?.id != null &&
            authorId != null &&
        ...
        <Box>
            ...
            {!isCommentsLoading && !isCommentsError &&
                comments.map((comment) => {
                    console.log("댓글 전체 데이터 확인:", comment); // 여기서 author가 있는지, authorId가 있는지 확인!

                    const { id, content, createdAt, member } = comment;
                    // 본인 댓글 여부 확인

                    const loginedEdit = checkEdit(member?.id);
